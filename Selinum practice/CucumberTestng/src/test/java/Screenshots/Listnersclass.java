...
import Actitime.TestCases.ActitimeBaseclass;
...
public class Listnersclass implements ITestListener {
	...
	public void onTestFailure(ITestResult result) {
		...
		TakesScreenshot sh=(TakesScreenshot)ActitimeBaseclass.driver;
		File src=sh.getScreenshotAs(OutputType.FILE);
		File  drs=new File("D:\\MyFramewok\\TestNgreports\\Screenshots\\"+result.getName()+".png");
		try {
			Files.copy(src, drs);
			Allure.addAttachment(result.getName(), new ByteArrayInputStream(((TakesScreenshot)ActitimeBaseclass.driver).getScreenshotAs(OutputType.BYTES)));
		} catch (IOException e) {
